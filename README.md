...
NN_Numpy_Class.py is a neural network class it can be used on other project it is not linked to the mnist dataset, the main functions to use it are:<br />
train_batch, to train it input a x by n numpy array, x being the batch size and n the inputs and the same for the targets array<br />
test_batch to get the loss of a batch on a test dataset<br />
forward : to use the trained model, you must feed it a 1 by n numpy array and it return a 1 by n output array<br />
